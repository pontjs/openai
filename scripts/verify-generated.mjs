...
const directory = resolve("src/apis/openai");
...
assert.equal(Object.keys(lock.apis).length, 288);
assert.equal(Object.keys(lock.components.schemas).length, 1421);
assert.deepEqual(lock, canonical, "api-lock.json is stale; run pnpm pontx with --use-remote");
assert.equal(lock.tags.length, 36);
...
console.log(`Generated-source gate passed: ${Object.keys(expected).length} deterministic files and 36 explicit Controllers.`);
