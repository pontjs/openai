...
const temporary = await mkdtemp(join(tmpdir(), "pontx-openai-pack-"));

try {
  const { stdout } = await execFileAsync("npm", ["pack", "--json", "--pack-destination", temporary], {
    cwd: root,
  ...
  for (const expected of [
    "LICENSE", "NOTICE", "README.md", "dist/index.d.ts", "dist/index.js", "dist/index.mjs",
    "dist/bin/api-lock.json", "dist/bin/cli.cjs", "package.json",
  ]) assert(files.has(expected), `missing npm artifact: ${expected}`);
  for (const forbidden of ["pontx-spec.json", "source.provenance.json", "pnpm-lock.yaml", "reports/unit-tests.json"]) {
  ...
  await execFileAsync(process.execPath, ["--input-type=module", "--eval", [
    ...
    "if (typeof sdk.createOpenAiClient !== 'function') process.exit(1);",
    "if (!sdk.default || !sdk.default.models) process.exit(2);",
    "});",
  ].join("")], { cwd: temporary });
  const cli = join(temporary, "node_modules/.bin/pontx-openai");
